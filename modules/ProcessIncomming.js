        ...
        if (albumInfoPath) {
          ...
          .then(owner =>  {
            if (owner) {
              ...
              throw new Error('Not an valid email address: ' + albumInfo.from);
            }
          })
          .then(album => {
          ...
          .then(imageResourceType =>  {
            ...
            async.each(files, function(file, callback) {
              ...
              if (pathUtil.extname(file).toLowerCase() === '.jpg') {
                ...
                .then(uploadInfo => {
                  resource.onlineUrl = uploadInfo.Location;
                  return mkdirp(pathUtil.dirname(resource.localPath));
                })
                .then(() => {
                  gm(fileData)
                    ...
                    if (err) {
                      ...
                      exec('exiftran -ai \'' + resource.localPath + '\'', function() {
                        logger.info('[ProcessIncomming] Updating timestamp in image %s to %s',
                          resource.localPath,
                          resource.created);
                        fs.utimes(resource.localPath, new Date(resource.created) / 1000, new Date(resource.added) / 1000, function(err) {
                          if (err) {
                          ...
                          resource.save()
                          .then(imageResource => {
                            logger.info('[ProcessIncomming] Resource saved to DB: %s',
                              resource);
              ...
              if (err) {
                reject(util.format('[ProcessIncomming] Unable to to process %s, error: %s',
                  filePath, err)
                );
              } else {
                fulfill();
          ...
          logger.warn('[ProcessIncomming] Unable to find info.json in %s',
            filePath);
          reject('[ProcessIncomming] Unable to find info.json');
        }
      });
    })
    .then(() => {
      logger.warn('[ProcessIncomming] All done')
  ...
  let backup = function(fileBuffer, path) {
    return new Promise(function(fulfill, reject) {
      if (config.amazon.backup) {
        ...
        hash.update(path);
        let key = util.format('images/%s%s', hash.digest('hex'), pathUtil.extname(path));
        logger.debug('[ProcessIncomming] Uploading %s to AWS S3 with Key: %s', path, key);
        s3.upload({
          Key: key,
          Body: fileBuffer,
